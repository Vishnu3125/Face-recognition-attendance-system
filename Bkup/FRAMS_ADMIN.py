...
#    sb.iconbitmap('FRAMS.ico')
    ...
    #        MFW.iconbitmap('FRAMS.ico')
            ...
            def create_csv():
                # Save as a csv file
                csv_name='/home/pi/Desktop/frams/Attendance/Manually Attendance/'+fileName+'.csv'
                print(attendance)
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /open,"\home\pi\Desktop\frams\Attendance\Manually Attendance\"')  #open attendance sheet window
...
def take_img():
    ...
    if l1 == '':
        ...
        try:
            ...
            Notification.configure(text=res, bg="SpringGreen3", width=50, font=('times', 8, 'bold'))
...
def subjectchoose():
  ...
  #  windo.iconbitmap('FRAMS.ico')
    ...
    sub = tk.Label(windo, text="Enter Subject", width=15, height=2, fg="white", bg="blue2", font=('times', 12, ' bold '))
    sub.place(x=50, y=70)
...
#    tx.place(x=200, y=70)
    ...
    fill_a = tk.Button(windo, text="Fill Attendance", fg="white",command=Fillattendances, bg="deep pink", width=20, height=2, activebackground="Red", font=('times', 10, ' bold '))
...
def admin_panel():
    ...
    with open(cs, newline="") as file:
        ...
        for col in reader:
            ...
            for row in col:
                # i've added some styling
                label = tkinter.Label(root, width=12, height=1, fg="black", font=('times', 8, ' bold '),
                                      bg="lawn green", text=row, relief=tkinter.RIDGE)
                label.grid(row=r, column=c)
                c += 1
            r += 1
...
def trainimg():
    ...
    try:
        ...
        Notification.configure(text=q, bg="SpringGreen3", width=50, font=('times', 10, 'bold'))
        Notification.place(x=60, y=240)
        return

    res = "Model Trained"
    Notification.configure(text=res, bg="SpringGreen3", width=50, font=('times', 10, 'bold'))
    Notification.place(x=60, y=210)
...
if __name__ == '__main__':
    ...
    window.geometry('1280x720') #480x286
 ...
 #   window.iconbitmap('FRAMS.ico')
    ...
    message = tk.Label(window, text="Face-Recognition-Based-Attendance-Management-System", bg="dark blue", fg="snow", width=50,
                       height=2, font=('times', 13, 'italic bold '))

    message.place(x=20, y=2)

    Notification = tk.Label(window, text="All things good", bg="Green", fg="white", width=15,
                          height=2, font=('times', 12, 'bold'))

    lbl = tk.Label(window, text="Enter Enrollment", width=16, height=1, fg="black", bg="cyan", font=('times', 10, ' bold '))
    lbl.place(x=60, y=45)
    ...
    txt = tk.Entry(window, validate="key", width=24, bg="snow", fg="red", font=('times', 8, ' bold '))
    txt['validatecommand'] = (txt.register(testVal),'%P','%d')
    txt.place(x=200, y=45)

    lbl2 = tk.Label(window, text="Enter Name", width=16, fg="black", bg="cyan", height=1, font=('times', 10, ' bold '))
    lbl2.place(x=60, y=75)

    txt2 = tk.Entry(window, width=24, bg="snow", fg="red", font=('times', 8, ' bold '))
    txt2.place(x=200, y=75)

    clearButton = tk.Button(window, text="Clear",command=clear,fg="black"  ,bg="cyan"  ,width=6  ,height=0 ,activebackground = "Red" ,font=('times', 8, ' bold '))
    clearButton.place(x=350, y=45)

    clearButton1 = tk.Button(window, text="Clear",command=clear1,fg="black"  ,bg="cyan"  ,width=6 ,height=0, activebackground = "Red" ,font=('times', 8, ' bold '))
    clearButton1.place(x=350, y=75)

    AP = tk.Button(window, text="Check Register students",command=admin_panel,fg="black"  ,bg="grey"  ,width=19 ,height=1, activebackground = "Red" ,font=('times', 10, ' bold '))
    AP.place(x=170, y=105)

    takeImg = tk.Button(window, text="Take Images",command=take_img,fg="white"  ,bg="grey",borderwidth=2  ,width=17  ,height=1, activebackground = "Red" ,font=('times', 10, ' bold '))
    takeImg.place(x=70, y=145)

    trainImg = tk.Button(window, text="Train Images",fg="white",command=trainimg ,bg="grey",borderwidth=2  ,width=17  ,height=1, activebackground = "Red" ,font=('times', 10, ' bold '))
    trainImg.place(x=250, y=145)

    FA = tk.Button(window, text="Automatic Attendance",fg="white",command=subjectchoose  ,bg="grey",borderwidth=2  ,width=17  ,height=1, activebackground = "Red" ,font=('times', 10, ' bold '))
    FA.place(x=70, y=180)

    quitWindow = tk.Button(window, text="Manually Fill Attendance", command=manually_fill  ,fg="white",borderwidth=2 ,bg="grey"  ,width=20  ,height=1, activebackground = "Red" ,font=('times', 9, ' bold '))
    quitWindow.place(x=250, y=180)

  #  Stats = tk.Button(window, text="Statistics", fg="white", command=stats, bg="grey", borderwidth=2,
    #               width=9, height=1, activebackground="Red", font=('times', 10, ' bold '))
